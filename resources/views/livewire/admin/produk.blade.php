                        ...
                        <div class="row">
                          ...
                          <div class="col-lg-6">
                            <div class="mb-3">
                              ...
                              <input wire:model="price" type="number" min="500" step="500" placeholder="0"
                                class="form-control">
                              @error('price')
                                <small class="text-danger">{{ $message }}</small>
                              @enderror
                          ...
                          <div class="col-lg-6">
                            <div class="mb-3">
                              ...
                              <input wire:model="image" type="file" class="form-control gambar-input"
                                accept="image/*">
                              @error('image')
                          ...
                          <div class="col-lg-6">
                            <div class="mb-3">
                              ...
                              @php
                                $img = $this->image ? $this->image->temporaryUrl() : asset('img/no-image.jpg');
                              @endphp
                              <img src="{{ $img }}" class="img-fluid d-block rounded border img-preview"
              ...
              <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                  <div class="modal-header">
                    <h5 class="modal-title" id="modalEditLabel">
                      Hapus Data</h5>
                    <button wire:click="resetInputFields" type="button" class="btn-close btn-close-delete"
              ...
              <tbody>
                @forelse ($items as $index => $item)
                  <tr>
                    ...
                    <td>
                      @if ($item->image)
                        <img class="img-fluid rounded border" src="{{ asset("storage/$item->image") }}"
                          style="width: 80px; height: auto; aspect-ratio: 16/9; object-fit: cover;" alt="preview">
                      @else
                        <img class="img-fluid rounded border" src="{{ asset('img/no-image.jpg') }}"
                          style="width: 80px; height: auto; aspect-ratio: 16/9; object-fit: cover;" alt="preview">
                      @endif
                    </td>
                    <td>{{ $item->name }}</td>
                    ...
                    <td>
                      ...
                      <sup data-bs-toggle="tooltip" title="Naik 5% dari harga awal" class="text-success"
                        style="font-weight: 900;">+5%</sup>
                    </td>
                    <td class="text-nowrap">
                      ...
                      <button wire:loading.attr="disabled" wire:target="getDeleteItem({{ $item->id }})"
                        wire:click="getDeleteItem({{ $item->id }})" wire:key="delete-item-{{ $item->id }}"
                ...
                @empty
                  <tr>
                    <td colspan="7" class="text-center text-muted">
                      {{ $this->search ? 'Data yang dicari tidak ditemukan.' : 'Belum ada data item.' }}</td>
                  </tr>
                @endforelse
              </tbody>
            </table>
