                                    ...
                                    @foreach ($carts as $cart)
                                        <tr class="table-body-row">
                                            <td class="product-remove">
                                                <button wire:click="deleteCartItem({{ $cart->id }})"
                                                    wire:loading.attr="disabled"
                                                    wire:target="deleteCartItem({{ $cart->id }})"
                                                    wire:key="cart-delete-({{ $cart->id }})"
                                            ...
                                            <td class="product-quantity">
                                                <input wire:model.live="quantities.{{ $cart->id }}"
                                                    wire:key="quantity-{{ $cart->id }}" style="outline: none;"
                                                    min="1" type="number" placeholder="0">
                                            </td>
                        ...
                        <div class="total-section">
                            ...
                            <div class="cart-buttons">
                                @if (!Auth::user()->contact || !Auth::user()->address)
                                    <button wire:click="checkProfil" class="btn-custom">
                                        Check Out</button>
                                @else
                                    <button href="{{ route('front.checkout') }}" wire:navigate class="btn-custom">
                                        Check Out
                                    </button>
                                @endif
                            </div>
